...
def select_folder():
    """
    Prompts the user to select a main folder containing subfolders.
    """
    Tk().withdraw()
    ...
    try:
        ...
        for filename in os.listdir(pdf_folder_path):
            if filename.lower().endswith(".pdf"):  # Check if the file is a PDF
                ...
                # Open the PDF to check for "sirius" or "XHS"
                doc = pymupdf.open(pdf_path)
                text = doc.load_page(0).get_text("text").lower()
                is_sirius_or_xhs = "sirius" in text or "xhs" in text

                # Add the PDF to the appropriate list based on the flag AND "sirius"/"XHS"
                if is_sirius_or_xhs:  # Only process files containing "sirius" or "XHS"
                    if as_found:
                    ...
                    print(f"Skipping file: {filename} (does not contain 'sirius' or 'XHS')")
...
def rename_pdf(folder_path, filename, unit_id=None, serial_number=None):
    """
    ...
    """
    pdf_path = os.path.join(folder_path, filename)
    if unit_id is None or serial_number is None:
...
def rename_subfolders(folder_path):
    """
    Renames subfolders based on the ID and serial number extracted from the files.
    """
    pdf_files = [f for f in os.listdir(folder_path) if f.lower().endswith(".pdf")]

    if pdf_files:
        first_pdf_path = os.path.join(folder_path, pdf_files[0])
        _, _, _, _, unit_id, serial_number = extract_slot_and_flags(first_pdf_path)

        cleaned_unit_id = re.sub(r'[<>:"/\\|?*].*', '', unit_id)

        if cleaned_unit_id and serial_number:
            new_folder_name = f"{cleaned_unit_id}_{serial_number}"
            parent_dir = os.path.dirname(folder_path)
            new_path = os.path.join(parent_dir, new_folder_name)

            print(f"Renaming folder {folder_path} to {new_path}")

            try:
                os.rename(folder_path, new_path)
                print(f"Folder renamed to: {new_folder_name}")
            except FileExistsError:
                print(f"Folder with name {new_folder_name} already exists.")
            except Exception as e:
                print(f"Error renaming folder: {e}")
        else:
            print("Could not extract ID and serial number from the PDF.")
    else:
...
if __name__ == "__main__":
    ...
    for folder_path, _, filenames in os.walk(main_folder_path):
